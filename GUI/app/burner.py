...
directory_path = current_directory  # Replace with the actual directory path
...
if not os.path.exists(directory_path):
    ...
    latest_file = max([f for f in files if os.path.isfile(os.path.join(directory_path, f))], key=lambda f: os.path.getmtime(os.path.join(directory_path, f)))

    if latest_file:
        print(f"The latest file in '{directory_path}' is: {latest_file}")
    else:
...
def CalulateBinFileLength():
    ...
    BinFileLength = os.path.getsize(latest_file)
...
def Decode_CBL_Command(Command):
    ...
    if(Command == 1):
        ...
        #SectorNumber = input("\n   Please enter start sector number(0-7)          : ")
        ...
        if(SectorNumber != 0xFF):
            ...
            #NumberOfSectors = int(input("\n   Please enter number of sectors to erase (8 Max): "), 16)
...
if __name__ == "__main__":
    ...
    #SerialPortName = input("Enter the Port Name of your device(Ex: /dev/ttyUSB0):")

    Serial_Port_Configuration("/dev/ttyUSB0")
